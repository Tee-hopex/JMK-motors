...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    fetch("/api/settings")
      ...
      .then((data) => {
        if (data.whatsapp) {
          // Remove non-digit characters and ensure it's in international format
          const cleaned = data.whatsapp.replace(/\D/g, "");
          setWhatsappNumber(cleaned);
        }
      })
      .catch(() => {}); // Use default if API fails
  }, []);
  ...
  return (
    ...
    >
      ...
      {/* Button */}
      <div className="wa-pulse w-14 h-14 bg-[#25D366] rounded-full flex items-center justify-center shadow-lg hover:scale-110 transition-transform duration-200">
        <svg
          viewBox="0 0 24 24"
